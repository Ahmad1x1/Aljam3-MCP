...
function createMcpServer() {
  const server = new McpServer({ name: 'aljam3-mcp', version: '1.0.0' });
      ...
      try {
        ...
        if (expand_files && data.files?.data?.length) {
          const files = data.files.data.map((f, i) =>
            `  [${i + 1}] 📁 ${f.format || 'file'}  (file_id: ${f.id})` +
            (f.pdf_url ? `\n       🔗 PDF: ${f.pdf_url}` : '') +
            (f.txt_url ? `\n       🔗 TXT: ${f.txt_url}` : '')
          ).join('\n');
...
app.listen(PORT, () => {
  console.log(`✅ Aljam3 MCP Server v1.0 aktif di port ${PORT}`);
  console.log(`   Endpoint MCP : http://localhost:${PORT}/mcp`);
  console.log(`   Health check : http://localhost:${PORT}/`);
  console.log(`   Tools aktif  : 10 tools`);
});
